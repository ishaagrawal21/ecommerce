...
const getProducts = async (req, res) => {
  try {
    ...
    const result = await productModel.find(filter).sort(sortOptions).populate("category", "name");
...
const createProduct = async (req, res) => {
  try {
    ...
    if (req.headers["content-type"]?.includes("multipart/form-data")) {
      ...
      const stock = fields.stock !== undefined ? fields.stock : req.body?.stock;
  ...
  try {
    ...
    if (req.headers["content-type"]?.includes("multipart/form-data")) {
      ...
      productData = {
        ...
        description: String(description).trim(),
        price: Number(price),
        stock: stock !== undefined && stock !== "" ? Number(stock) : (existingProduct.stock ?? 10),
        category: categoryId,
      };
